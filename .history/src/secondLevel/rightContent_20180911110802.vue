  ...
  <div class="rightContent-body">
    <div class="rightContent-body-header">
      <ButtonGroup size="large">
        ...
        <Button type="ghost" icon="ios-cloud-download-outline" @click.native="test">重置实验</Button><Icon type="refresh"></Icon>
      </ButtonGroup>
    </div>
...
<script type="text/ecmascript-6">
  ...
  import { Table , RadioGroup , Radio ,Icon ,Button ,Spin ,Alert ,Message ,Modal ,ButtonGroup} from 'iview';
  ...
  export default {
    ...
    methods: {
      ...
      realRadioCustom(){
        ...
        let a = {
          ...
          render: (h, params)=> {
            ...
            let isChecked = false;
            if (factors.length) {//这里不能forEach,否则渲染不出RadioGroup组件，我也不知道为什么 = =!
                    ...
                    nativeOn: {//触发组件事件名
                      change: (e)=> {
                        self.text.forEach((item)=> {
                          if (item.value == $.trim(e.path[6].children[1].innerText)) {
      ...
      extraction (){
        ...
        $.post('http://101.200.12.178:8090/OFTPService/services/Sample/extract', {'para': para})
          .done((data)=> {
      ...
      sample(){
        //拼接预报时间
        let dateYear = this.$store.state.firstCache.yearStart
          + '年-' + this.$store.state.firstCache.yearEnd + '年' +
          this.$store.state.firstCache.timeFrameStart +
          '至' + this.$store.state.firstCache.timeFrameEnd;
        //拼接站点信息
    ...
    computed: {
      ...
      getExtractionParam(){
        ...
        return {
          ...
          elementForecast: $.trim(this.$store.state.firstCache.predictionMsg.value),
          stations: stationArr,
          factors: FactorsArr,
