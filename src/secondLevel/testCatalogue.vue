    ...
    <div class="testDetails">
        <div class="testDetails_item">
          ...
          <span class="spanTitle">评估结果</span>
           <div class="testDetails_item_content">
            <Table border :columns="JSON.parse(details[0].testResult).cols" :data="JSON.parse(details[0].testResult).row" width="100%"></Table>
          </div>
          <!--预报方案-->
...
<script type="text/ecmascript-6">
  ...
  export default{
                ...
                h(Button, {
                  ...
                  on: {
                    click: () => {
                      ...
                      this.ID = id;
                      $.post(this.$host + 'Project/getProject', {para: JSON.stringify({id: id})})
                        .done(data => {
                          this.details = data;
                ...
                h(Button, {
                  ...
                  on: {
                    click: () => {
                      ...
                      let id = params.row.id;
                      $.post(this.$host + 'Project/deleteMethod', {para: JSON.stringify({id: id})})
                    }
                  }
                }, '删除')
              ]);
    ...
    created(){
      //接口获取实验目录
      $.post(this.$host + 'Project/getProjects', {para: JSON.stringify({testUser: '杨春'})})
        .done((data)=> {
