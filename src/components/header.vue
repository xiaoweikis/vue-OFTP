...
export default {
  ...
  methods: {
    ...
    loginvalidate(){
       ...
       $.post('http://101.200.12.178:8090/WMUser/services/UserService/login',{para:JSON.stringify(a)})
       .done( data => {
         if(data && data.length){
    ...
    updatePassword(){
       ...
       $.post('http://101.200.12.178:8090/WMUser/services/UserService/updatePassword',{para:JSON.stringify(a)})
       .done( data => {
         if(data){
           this.$Message.success({
              content:'密码修改成功'
