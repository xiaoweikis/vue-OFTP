...
<script type="text/ecmascript-6">
  ...
  import $ from 'jquery'
  import { ButtonGroup , Button ,Table ,CheckboxGroup ,Checkbox ,Message , Icon, Select , Option} from 'iview';
  ...
  export default {
    ...
    methods: {
      ...
      //提交需要预测的因子
      factorsCommit(){
        let tabelBody = $('.ivu-table-body').eq(0).find('input.ivu-checkbox-input');
        let value = [];
        ...
        $.post('http://101.200.12.178:8090/OFTPService/services/Sample/selectFactor', {para: JSON.stringify(a)})
          .done((data) => {
            if (data && data !== '') {
              Message.success({
      ...
      //获取相关分析接口数据
      getCorrelation (){
        axios.post('http://101.200.12.178:8090/OFTPService/services/Analysis/correlation')
          .then((data) => {
            this.CorrelationUrl = 'http://101.200.12.178:8090' + data.data.url;
            this.CorrelationData = data.data.result;
            this.initTableTwo();
            this.initTableThree();
